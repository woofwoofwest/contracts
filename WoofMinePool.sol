...
contract WoofMinePool is OwnableUpgradeable, PausableUpgradeable {
    ...
    function unstakeMiner(uint32 _tokenId, uint8 _index) external {
        ...
        stakedMiners[_tokenId][_index] = miners[miners.length - 1];
    ...
    function unstakeHunter(uint32 _tokenId) external {
        require(tx.origin == _msgSender());
        require(woofMine.ownerOf(_tokenId) == _msgSender(), "1");
        uint32 hunterId  = _unstakeHunter(_tokenId);
    ...
    function recoverStaminaOfMiner(uint32 _tokenId, uint32 _index, uint32 _stamina) external {
        ...
        IWoofMine.WoofWoofMine memory w = woofMine.getTokenTraits(_tokenId);
        IWoof.WoofWoofWest memory ww = woof.getTokenTraits(miner.tokenId);
        uint32 curStamina = _currentStamina(miner.depositTime, w, ww);
        if (curStamina == 0) {
            (uint256 pendingPaw, uint256 pendingGem) = _pendingRewards(miner, w, ww);
        ...
        if (_stamina + curStamina > ww.attributes[1]) {
            _stamina = ww.attributes[1];
        } else {
            _stamina += curStamina;
        }
    ...
    function recoverStaminaOfHunter(uint32 _tokenId, uint32 _stamina) external {
        ...
        uint32 curStamina = _currentStamina(hunter.depositTime, w, ww);
       if (_stamina + curStamina > ww.attributes[1]) {
            _stamina = ww.attributes[1];
        } else {
            _stamina += curStamina;
    ...
    function woofMineBrief(uint32 _tokenId) public view returns(IWoofMineEnumerable.WoofMineBrief memory) {
        ...
        brief.pendingPaw = mineReward[_tokenId].pendingPaw;
        brief.pendingGem = mineReward[_tokenId].pendingGem;
        brief.lastRewardTime = mineReward[_tokenId].lastRewardTime;
        StakedMiner[] memory miners = stakedMiners[_tokenId];
        if (miners.length > 0) {
        ...
        for (uint256 i = 0; i < miners.length; ++i) {
            ...
            brief.miners[i].stamina = _currentStamina(miner.depositTime, w, ww);
        ...
        if (hunter.tokenId > 0) {
            ...
            brief.hunter.stamina = _currentStamina(hunter.depositTime, w, ww);
    ...
    function _unstakeAllMiners(uint32 _tokenId) internal {
        StakedMiner[] memory miners = stakedMiners[_tokenId];
        if (miners.length == 0) {
            return;
        }
    ...
    function _unstakeHunter(uint32 _tokenId) internal returns(uint32) {
        StakedHunter memory hunter = stakedHunter[_tokenId];
        if (hunter.tokenId == 0)  {
            return 0;
        }
        ...
        woof.transferFrom(address(this), _msgSender(), hunter.tokenId);
    ...
    function _claim(StakedMiner memory _miner, IWoofMine.WoofWoofMine memory _woofMine, IWoof.WoofWoofWest memory _woofWest) internal returns(uint256 pendingPaw, uint256 pendingGem) {
        (pendingPaw, pendingGem) = _pendingRewards(_miner, _woofMine, _woofWest);
    ...
    function _updateStamina(uint32 _depositTime, IWoofMine.WoofWoofMine memory _woofMine, IWoof.WoofWoofWest memory _woofWest) internal {
        uint32 stamina = _currentStamina(_depositTime, _woofMine, _woofWest);
        woof.updateStamina(_woofWest.tokenId, stamina);
    }

    function _currentStamina(uint32 _depositTime, IWoofMine.WoofWoofMine memory _woofMine, IWoof.WoofWoofWest memory _woofWest) internal view returns(uint32) {
    ...
    function _pendingRewards(StakedMiner memory _miner, IWoofMine.WoofWoofMine memory _woofMine, IWoof.WoofWoofWest memory _woofWest) internal view returns(uint256 pendingPaw, uint256 pendingGem) {
