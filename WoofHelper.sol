...
contract WoofHelper is  OwnableUpgradeable, PausableUpgradeable {
    ...
    function initialize(
        ...
        nameCost = 1e18;
        batchMintBanditProbability = 80;
        strictRewardMine = false;
    ...
    function batchMint(address _referrer) external whenNotPaused {
        require(tx.origin == _msgSender(), "Not EOA");
        uint256 paymentAmount = mintPrice;
        if (startGemMint == false) {
        ...
        for (uint8 i = 0; i < 10; ++i) {
            ...
            if (i == 9 && mintBanditOrHunter == false) {
                ...
                uint8 race = uint8(seeds[1] % 2);
                uint8 gender = uint8(seeds[2] % 2);
                uint8 quality = uint8((seeds[3] % 100 < 60) ? 0 : 1);
                w = config.randomWoofWoofWest(seeds, nftType, quality, race, gender);
            } else {
                w = config.randomWoofWoofWest(seeds);
            }
            w.tokenId = uint32(_minted);
            w.generation = (startGemMint == false) ? 0 : 1;
    ...
    function recoverStamina(uint32 _tokenId, uint32[] memory _itemIds, uint32[] memory _amount) external {
        ...
        if (w.attributes[0] + stamina > w.attributes[1]) {
            stamina =  w.attributes[1];
        } else {
            stamina = w.attributes[0] + stamina;
        }
    ...
    function _mintWoofMine() internal {
        ...
        if (startGemMint == false && rewardMineCount <= G0_MAX_MINT_AMOUNT / 10 && woofMine != address(0)) {
            rewardMineCount += 1;
    ...
    function mintDashboard(address _user) public view returns(
        ...
        minted_ = _minted;
        startG1Mint_ = startGemMint;
        if (_user != address(0)) {
            if (startG1Mint_ == false) {
                discount_ = discount.discount(_user);
