...
contract Wanted is OwnableUpgradeable, PausableUpgradeable {
    ...
    function initialize(
        ...
        perPowerMaxClaimGemAmount = 1e16;
    ...
    function wantedBrief(uint32 _hunterId) public view returns(uint8 wantedCount, uint8 maxWantedCount) {
        ...
        require(hunter.nftType == 2, "Not hunter");
    ...
    function wantedBandit(uint32 _hunterId, uint32 _banditId) external whenNotPaused {
        ...
        require(woof.ownerOf(_hunterId) == _msgSender(), "Not owner");
        require(woof.ownerOf(_banditId) != _msgSender(), "Can't wanted self bandit");

        IWoof.WoofWoofWest memory hunter = woof.getTokenTraits(_hunterId);
        require(hunter.nftType == 2, "Not hunter");
        require(hunter.attributes[0] >= costStaminaOfWanted, "Not enough stamina");
        ...
        require(bandit.nftType == 1, "Not bandit");

        WantedBanditInfo[] memory wantedBandits = wantedBanditInfo[_hunterId];
        require(wantedBandits.length <= maxWantedCountOfLevel[hunter.level - 1], "Reach the max wanted count");
        for (uint8 i = 0; i < wantedBandits.length; ++i) {
            require(wantedBandits[i].banditId != _banditId, "Exist wanted bandit");
        }

        uint256 miningPower = woof.miningPowerOf(hunter);
        BanditPayInfo memory payInfo = banditPayInfo[_banditId];
        require(payInfo.hunters.length <= maxWantedCountOfBandit, "Reach the max wanted count of bandit");
    ...
    function freeBandit(uint32 _hunterId, uint8 _pos) external {
        ...
        IWoof.WoofWoofWest memory hunter = woof.getTokenTraits(_hunterId);
        require(hunter.nftType == 2, "Not hunter");
        require(hunter.attributes[0] >= costStaminaOfFree, "Not enough stamina");
        hunter.attributes[0] -= costStaminaOfFree;
        woof.updateStamina(_hunterId, hunter.attributes[0]);

        WantedBanditInfo[] memory wantedBandits = wantedBanditInfo[_hunterId];
        require(_pos < wantedBandits.length, "Invalid pos");
        uint256 miningPower = woof.miningPowerOf(hunter);
    ...
    function updateTokenTraits(IWoof.WoofWoofWest memory _w, uint256 _beforeMiningPower) external {
        require(authControllers[_msgSender()], "no auth");
        if (_w.nftType != 2) {
            return;
        }
        WantedBanditInfo[] memory wantedBandits = wantedBanditInfo[_w.tokenId];
        if (wantedBandits.length == 0) {
            return;
        }
        uint256 miningPower = woof.miningPowerOf(_w);
        for (uint8 i = 0; i < wantedBandits.length; ++i) {
            WantedBanditInfo memory wantedInfo = wantedBandits[i];
            BanditPayInfo memory payInfo = banditPayInfo[wantedInfo.banditId];
            ...
            payInfo.totalMiningPower = payInfo.totalMiningPower.add(miningPower).sub(_beforeMiningPower);
            banditPayInfo[wantedInfo.banditId] = payInfo;
        }
    }
    ...
    function _claimOrFreeAll(uint32 _hunterId, bool _free) internal {
        ...
        require(hunter.attributes[0] >= costStamina, "Not enough stamina");
        hunter.attributes[0] -= costStamina;
        woof.updateStamina(_hunterId, hunter.attributes[0]);

        WantedBanditInfo[] memory wantedBandits = wantedBanditInfo[_hunterId];
        require(wantedBandits.length > 0, "No wanted bandit");
        uint256 miningPower = woof.miningPowerOf(hunter);
