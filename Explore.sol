...
contract Explore is IExplore, OwnableUpgradeable, PausableUpgradeable {
    ...
    function endExplore(uint32 _nftId) public {
        ...
        require(info.owner == _msgSender(), "Not Owner");
        ...
        uint256[] memory seeds = randomseeds.multiRandomSeeds(block.timestamp + _nftId, 4);
        ...
        uint256 probability = power * 100 / probabilityBufOfPower;
        probability = probability + eEvent.result1Probability;
        if (seeds[0] % 1000 < probability) {
            record.result = 1;
            record.rewardAmount = eEvent.rewardOfResult1[0] + seeds[1] % (eEvent.rewardOfResult1[1] - eEvent.rewardOfResult1[0]);
            record.dropItems = _dropItems(eEvent.dropItemProbabilityOfResult1, seeds[2], eEvent.dropItems);
            record.dropEquipmentBlindBox = _dropEquipmentBlindBox(eEvent.dropEquipBoxProbability, seeds[3]);
            if (eEvent.rewardBaby == true) {
                record.rewardBabyId = baby.mint(_msgSender(), _nftId);
            }
        } else {
            record.result = 2;
            record.rewardAmount = eEvent.rewardOfResult2[0] + seeds[1] % (eEvent.rewardOfResult2[1] - eEvent.rewardOfResult2[0]);
            record.dropItems = _dropItems(eEvent.dropItemProbabilityOfResult2, seeds[2], eEvent.dropItems);
        }
        woof.updateTokenTraits(w);
        if (record.rewardAmount > 0) {
            if (config.costToken == 0) {
                (record.rewardAmount,) = _paySlaveTax(_nftId, record.rewardAmount, 0);
                IPaw(paw).mint(_msgSender(), record.rewardAmount);
                userDashboard.rewardFromScene(_msgSender(), 4, record.rewardAmount, 0, 0);
            } else {
                (,record.rewardAmount) = _paySlaveTax(_nftId, 0, record.rewardAmount);
                gemMintHelper.mint(_msgSender(), record.rewardAmount);
                userDashboard.rewardFromScene(_msgSender(), 4, 0, record.rewardAmount, 0);
            }
        }
        ...
        _removeUserNft(_msgSender(), _nftId);
        delete nftIdToInfo[_nftId];
        emit EndExplore(_msgSender(), _nftId, rid);
