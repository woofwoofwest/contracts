...
contract Barn is OwnableUpgradeable, PausableUpgradeable {
    ...
    function stakeWoof(uint32 _tokenId) external whenNotPaused {
        ...
        require(tx.origin == user, "Not EOA");
        require(_tokenId > 0, "Invalid token id");
        require(woof.ownerOf(_tokenId) == user, "Not owner");
    ...
    function unstakeWoof(uint32 _tokenId) external {
        ...
        uint256 taxAmount = 0;
        if (w.nftType == 1)  {
            rewardAmount = _unstakeBandit(w, user);
        } else {
            (rewardAmount, taxAmount) = _unstakeMinerOrHunter(w, user);
        }
        _removeUserNft(user, _tokenId);
        if (mapUserNfts[user].length == 0) {
            delete mapUserNfts[user];
        }
        woof.transferFrom(address(this), user, _tokenId);
        stakedAmountOfEachType[w.nftType] -= 1;
        emit UnStakeWoof(user, _tokenId, rewardAmount, taxAmount);
    }
    ...
    function unstakeWoofs(uint32[] calldata _tokenIds) external {
        require(_tokenIds.length <= 10, "Unstake too many");
        address user = _msgSender();
        require(tx.origin == user, "Not EOA");
    ...
    function claimWoofs(uint32[] calldata _tokenIds) external {
        require(_tokenIds.length <= 10, "Claim too many");
        address user = _msgSender();
        require(tx.origin == user, "Not EOA");
    ...
    function _unstakeMinerOrHunter(IWoof.WoofWoofWest memory _w, address _user) internal returns(uint256,  uint256) {
        ...
        require(staked.owner == _user, "Not owner");
    ...
    function _claimOrUnstakeMany(bool _unstake, address _user, uint32[] calldata _tokenIds) internal {
        require(mapUserNfts[_user].length > 0, "No staked nft");
